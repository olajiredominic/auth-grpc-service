...
func New(db *gorm.DB, clientID, secretKey, tokenURL, qoreidBaseURL, vninURL, ninURL string) Handler {
	return Handler{
		DB:            db,
		ClientID:      clientID,
		SecretKey:     secretKey,
...
func Init(url, clientID, secretKey, tokenURL, qoreidBaseURL, vninURL, ninURL string) Handler {
